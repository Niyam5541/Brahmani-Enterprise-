...
function productPhoto(p){return `<div class="product-icon" aria-hidden="true">${iconFor(p.category)}</div>`}
function renderProducts(){els.grid.innerHTML='';filtered.slice(0,visible).forEach(p=>{const card=document.createElement('article');card.className='product-card';card.innerHTML=`${productPhoto(p)}<div class="product-category">${p.category}</div><h3 class="product-name">${p.name}</h3><div class="product-meta">Catalogue page ${p.page} • Water Jet Loom Spare Part</div><div class="qty-row"><input type="number" min="1" value="1" aria-label="Quantity"><button type="button">Add to Order</button></div>`;const inp=card.querySelector('input');card.querySelector('button').onclick=()=>addCart(p,Math.max(1,parseInt(inp.value)||1));els.grid.appendChild(card)});els.count.textContent=`${filtered.length} part${filtered.length===1?'':'s'} found`;els.load.style.display=visible>=filtered.length?'none':'block'}
