...
function productPhoto(p){const m=(window.PRODUCT_IMAGE_MAP||{})[`${p.name}|${p.page}`];if(!m)return `<div class="product-photo fallback"><span>${iconFor(p.category)}</span></div>`;const [pg,row,col]=m;const x=col*180,y=row*117;return `<div class="product-photo"><div class="sprite-crop" role="img" aria-label="${p.name.replace(/"/g,'&quot;')}" style="background-image:url('${(window.SPRITES||{})[pg]||''}');background-position:-${x}px -${y}px"></div></div>`}
