...
function productPhoto(p){const enhanced=(window.ENHANCED_PRODUCT_IMAGES||{})[p.name];if(enhanced)return `<div class="product-photo"><img src="${enhanced}" alt="${p.name.replace(/"/g,'&quot;')}" loading="lazy" style="width:100%;height:100%;object-fit:contain;background:#fff"></div>`;const m=(window.PRODUCT_IMAGE_MAP||{})[`${p.name}|${p.page}`];if(!m)return `<div class="product-photo fallback"><span>${iconFor(p.category)}</span></div>`;const [pg,row,col]=m;const x=col*180,y=row*117;return `<div class="product-photo"><div class="sprite-crop" role="img" aria-label="${p.name.replace(/"/g,'&quot;')}" style="background-image:url('${(window.SPRITES||{})[pg]||''}');background-position:-${x}px -${y}px"></div></div>`}
function renderProducts(){els.grid.innerHTML='';filtered.slice(0,visible).forEach(p=>{const card=document.createElement('article');card.className='product-card';card.innerHTML=`${productPhoto(p)}<div class="product-category">${p.category}</div><h3 class="product-name">${p.name}</h3><div class="product-meta">Catalogue page ${p.page} • Photo from catalogue</div><div class="qty-row"><input type="number" min="1" value="1" aria-label="Quantity"><button type="button">Add to Order</button></div>`;const inp=card.querySelector('input');card.querySelector('button').onclick=()=>addCart(p,Math.max(1,parseInt(inp.value)||1));els.grid.appendChild(card)});els.count.textContent=`${filtered.length} part${filtered.length===1?'':'s'} found`;els.load.style.display=visible>=filtered.length?'none':'block'}
